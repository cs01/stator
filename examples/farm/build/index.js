...
var _statorgfc = __webpack_require__(15);
...
_statorgfc.store.initialize({
  numSheep: 10,
  numWolves: 2,
  numChickens: 90
});
_statorgfc.store.use(_statorgfc.middleware.logChanges);

// Create a normal component

var SheepWatcher = function (_React$Component) {
  _inherits(SheepWatcher, _React$Component);

  function SheepWatcher() {
    _classCallCheck(this, SheepWatcher);

    // connect global store to the state of this component
    var _this = _possibleConstructorReturn(this, (SheepWatcher.__proto__ || Object.getPrototypeOf(SheepWatcher)).call(this));

    _statorgfc.store.connectComponentState(_this, ['numSheep', 'numWolves']); // this.setState will be called when 'numSheep' or 'numWolves' changes
    return _this;
  }

  _createClass(SheepWatcher, [{
    key: 'render',
    value: function render() {
      return _react2.default.createElement(
        ...
        'Current sheep status: ',
        this.state.numSheep > this.state.numWolves ? 'all good' : 'watch out sheep!'
      );
    }
  }, {
    key: 'componentDidMount',
    value: function componentDidMount() {
      console.warn('unwatched keys of the store', _statorgfc.store.getUnwatchedKeys());
      console.log('subscribers to keys of the store', _statorgfc.store.getKeySubscribers());
    }
  }]);
...
_statorgfc.store.set({ numSheep: _statorgfc.store.get('numSheep') + 1 });
_statorgfc.store.set({ numChickens: 100 }); // doesn't trigger a call to setState in sheepWatcher
...
_reactDom2.default.render(_react2.default.createElement(SheepWatcher, null), document.getElementById('app'));
